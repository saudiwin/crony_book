...
combine_amces <- function(models,uncond=T) {
  ...
  if(uncond==T) {
    ...
    length_vars <- sapply(models[[1]]$estimates,function(m) {
      ncol(m[,!is.na(m[1,])])
    })
    ...
    check_nas <- lapply(models[[1]]$estimates, function(c)  { 
    ...
    length_vars <- sapply(models[[1]]$cond.estimates,function(m) {
      ncol(m[,!is.na(m[1,])])
    })
    ...
    check_nas <- lapply(models[[1]]$cond.estimates, function(c)  { 
  ...
  # check for missing coefs
  
  each_names <- lapply(1:length(each_names), function(n) {
    each_names[[n]][check_nas[[n]]]
  })
  ...
  # need to remove any missing coefficients (due to insufficient data)
  
  remove_list <- which(is.na(B[,2]))
  
  if(length(remove_list)>0) {
    B <- B[-remove_list,-remove_list]
    comb.est <- comb.est[-(remove_list-1)]
  }
